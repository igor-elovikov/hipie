...
static void volumestore3_impl(
    float3 value,
    int idx,
    global float* volume_x,
    global float* volume_y,
    global float* volume_z
)
{
    volume_x[idx] = value.x;
    volume_y[idx] = value.y;
    volume_z[idx] = value.z;
}
...
#define volumestore3(value, idx, name) volumestore3_impl(value, idx, name##_x, name##_y, name##_z)
